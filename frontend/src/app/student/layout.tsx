...
import { LayoutDashboard, Compass, History, LogOut, Menu, X, User } from "lucide-react";
      ...
      if (storedUser) {
        try {
          ...
          if (parsed.role !== "Student") {
            router.push(parsed.role === "Teacher" ? "/teacher/dashboard" : "/login");
          } else {
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 flex">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white/70 dark:bg-zinc-900/70 backdrop-blur-xl border-r border-zinc-200/50 dark:border-zinc-800/50 p-6 fixed h-full z-20">
        <div className="flex items-center gap-2.5 mb-8">
          <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-violet-600 to-indigo-600 flex items-center justify-center text-white font-extrabold text-xl shadow-lg shadow-violet-500/25">
            Q
          </div>
          <div>
            <h1 className="text-lg font-bold bg-gradient-to-r from-zinc-900 to-zinc-700 dark:from-zinc-50 dark:to-zinc-300 bg-clip-text text-transparent">
              QuizNova <span className="text-xs bg-violet-500/10 text-violet-600 dark:text-violet-400 px-1.5 py-0.5 rounded-md ml-1 font-semibold">AI</span>
            </h1>
          </div>
        </div>
      ...
      <div className="md:hidden flex items-center justify-between w-full h-16 px-6 bg-white/70 dark:bg-zinc-900/70 backdrop-blur-xl border-b border-zinc-200/50 dark:border-zinc-800/50 fixed top-0 left-0 z-30">
        <div className="flex items-center gap-2">
          <div className="h-8 w-8 rounded-lg bg-gradient-to-tr from-violet-600 to-indigo-600 flex items-center justify-center text-white font-extrabold text-base">
            Q
          </div>
