  ...
  return (
    <div
      ...
      className={`cursor-pointer transition-all duration-300 rounded-2xl p-5 border shadow-sm ${
        active
          ? "bg-indigo-50/75 dark:bg-indigo-950/20 border-indigo-500/50 scale-[1.02] ring-2 ring-indigo-500/20"
          : "bg-white dark:bg-zinc-900 border-zinc-200/60 dark:border-zinc-800/60 hover:border-zinc-350 dark:hover:border-zinc-700"
      }`}
    >
...
export default function LibraryPage() {
  ...
  const getAllQuizzesRecursive = (fList: FolderData[]): any[] => {
    let list: any[] = [];
    fList.forEach((f) => {
      const folderQuizzes = f.quizzes.map((q) => ({ ...q, folderName: f.name, folderId: f.id }));
      list = [...list, ...folderQuizzes];
      if (f.subFolders && f.subFolders.length > 0) {
        list = [...list, ...getAllQuizzesRecursive(f.subFolders)];
      }
    });
    return list;
  };

  const allQuizzes = getAllQuizzesRecursive(folders);
  ...
  // Recursive folder item rendering
  const FolderItem = ({ folder, level = 0 }: { folder: FolderData; level: number }) => {
    ...
    // Filter quizzes inside folder based on selected tab card
    const filteredQuizzes = folder.quizzes.filter(
      (q) => statusFilter === "all" || q.status === statusFilter
    );

    return (
      <div className="border border-zinc-200/50 dark:border-zinc-800/50 rounded-2xl overflow-hidden shadow-sm bg-white dark:bg-zinc-900 transition-all duration-300">
        <div
          className="flex items-center gap-3 p-4 cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors group"
        ...
        >
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              onClick={(e) => {
                e.stopPropagation();
                setFolderModal({ open: true, parentFolderId: folder.id });
              }}
              title="Add Subfolder"
              className="h-8 w-8 rounded-lg bg-violet-50 dark:bg-violet-900/20 text-violet-600 flex items-center justify-center hover:bg-violet-100 transition-colors"
            >
              <FolderPlus size={14} />
            </button>
            ...
            <button
              ...
              title="Add Quiz"
              className="h-8 w-8 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 flex items-center justify-center hover:bg-indigo-100 transition-colors"
            >
              <Plus size={14} />
            </button>
              ...
              {folderMenuOpen === folder.id && (
                ...
                >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2.5 w-full px-4 py-2.5 text-sm text-red-650 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                  >
                    <Trash2 size={13} />
                    Delete Folder
                  </button>
        ...
        {isExpanded && (
          <div className="border-t border-zinc-150/60 dark:border-zinc-800/80 bg-zinc-50/30 dark:bg-zinc-950/10 space-y-2 p-2">
            {/* Subfolders list */}
            ...
            {/* Quizzes list */}
            {filteredQuizzes.length === 0 && (!folder.subFolders || folder.subFolders.length === 0) ? (
              <div className="p-4 text-center">
                <p className="text-zinc-400 text-xs mb-2">No quizzes match filter in this folder</p>
                <button
                  onClick={() => setAddQuizModal(folder.id)}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 text-[10px] font-semibold hover:bg-indigo-100 transition-colors"
                >
              ...
              <div className="divide-y divide-zinc-100 dark:divide-zinc-850">
                {filteredQuizzes.map((quiz) => (
                  <div
                    key={quiz.id}
                    className="flex items-center gap-3 px-5 py-2.5 hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors group/quiz rounded-lg"
                    style={{ paddingLeft: `${(level + 1) * 1.5 + 1.2}rem` }}
                  >
                    <FileText size={14} className="text-zinc-400 flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-zinc-850 dark:text-zinc-200 truncate">
                        {quiz.title}
                      </p>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span
                        ...
                        <span className="text-[9px] text-zinc-400">{quiz.questionCount} questions</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover/quiz:opacity-100 transition-opacity">
                      <Link
                        ...
                        className="h-7 w-7 rounded-lg bg-zinc-150/80 dark:bg-zinc-800 flex items-center justify-center hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:text-indigo-600 text-zinc-500 transition-colors"
                        title="Edit"
                      >
                        <Pencil size={12} />
                      </Link>
                      <Link
                        ...
                        className="h-7 w-7 rounded-lg bg-zinc-150/80 dark:bg-zinc-800 flex items-center justify-center hover:bg-blue-50 dark:hover:bg-blue-900/30 hover:text-blue-600 text-zinc-500 transition-colors"
                        title="Analytics"
                      >
                        <BarChart2 size={12} />
                      </Link>
                      ...
                      <div className="relative">
                        <button
                          ...
                          className="h-7 w-7 rounded-lg bg-zinc-150/80 dark:bg-zinc-800 flex items-center justify-center hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-500 transition-colors"
                        >
                        ...
                        {quizMenuOpen === quiz.id && (
                          ...
                          >
                            <Link
                              href={`/teacher/quizzes/edit/${quiz.id}`}
                              className="flex items-center gap-2.5 w-full px-4 py-2 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800"
                            >
                              <Pencil size={13} className="text-zinc-400" />
                              Edit Quiz
                            </Link>
                            ...
                            <button
                              ...
                              className="flex items-center gap-2.5 w-full px-4 py-2 text-sm text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20"
                            >
                              <MoveRight size={13} />
                              Remove from Folder
                            </button>
                            <button
                              ...
                              className="flex items-center gap-2.5 w-full px-4 py-2 text-sm text-red-650 hover:bg-red-50 dark:hover:bg-red-900/20"
                            >
                              <Trash2 size={13} />
                              Delete Quiz
                            </button>
  ...
  // Filter root folder items
  const rootFolders = folders.filter((f) => !f.parentFolderId);

  // Stats filter logic
  const displayedFolders = rootFolders;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 dark:text-zinc-400 mt-1 text-sm">
            Organize your quizzes into folders and nested subfolders
          </p>
        </div>
        <button
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          ...
          <StatCard
            ...
            sub="Click to display published"
          />
              ...
              {searchResults.map((q) => (
                <div key={q.id} className="flex items-center gap-3 px-5 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-zinc-400">
                      in {q.folderName} · {q.questionCount} questions
                    </p>
                  </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-bold text-base text-zinc-700 dark:text-zinc-300">
              {statusFilter === "all" ? "All Folders" : `${statusFilter} Quizzes`} ({displayedFolders.length})
            </h2>
          </div>
          ...
          {displayedFolders.length === 0 ? (
            <div className="bg-white dark:bg-zinc-900 border border-dashed border-zinc-300 dark:border-zinc-700 rounded-3xl p-12 text-center">
              ...
              <h3 className="font-bold text-lg text-zinc-700 dark:text-zinc-300">No folders yet</h3>
              <p className="text-zinc-400 text-sm mt-1 mb-4">Create folders to organize your quizzes</p>
              <button
                onClick={() => setFolderModal({ open: true })}
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200/60 dark:border-zinc-800/60 w-full max-w-sm shadow-2xl p-6">
            ...
            <p className="text-center text-zinc-400 text-sm mt-2">
              {deleteConfirm.type === "folder"
                ? `"${deleteConfirm.name}" and all its subfolders/quizzes will be permanently deleted.`
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-red-650 text-white text-sm font-semibold hover:bg-red-700 transition-colors"
              >
                Delete
              </button>
            </div>
