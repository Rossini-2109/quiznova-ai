...
/**
 * Top‑level Join page (/join)
 * Mirrors the existing student/join UI but only asks for the join code.
 * After validation it redirects the student to the lobby page
 * `/student/lobby/{sessionCode}` which displays the teacher's lobby.
 */
export default function JoinPage() {
  ...
  const handleJoin = async () => {
    ...
    try {
      setLoading(true);
      setError("");
      // Validate the code via backend – the same endpoint used in the student join page
      const res = await api.get(`/quiz/code/${code}`);
      // The API returns the sessionCode (might be the same as the provided code)
      const sessionCode = res.data.sessionCode ?? code;
    ...
    <div className="max-w-md mx-auto py-12 px-4">
      <div className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-800/50 rounded-3xl p-8 shadow-2xl shadow-violet-500/5 text-center flex flex-col items-center">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 text-xs mt-1 mb-8 max-w-xs mx-auto">
          Enter the access code provided by your instructor to start the assessment.
        </p>
